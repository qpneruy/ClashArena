...
public class WorldEditPaster extends AbstractSchematicPaster {

    private final ExecutorService executorService;

    public WorldEditPaster(ExecutorService executorService) {
        super();
    ...
    public CompletableFuture<Void> paste(File file, Location location, boolean ignoreAirBlock) {
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            try (EditSession editSession = WorldEdit.getInstance().newEditSession(clipboardData.worldEditWorld())) {
                ...
                ClashArenaLogger.debug(ClashArenaLogger.debugLevel(), Level.INFO, "WorldEdit paste completed for: " + file.getName());

            } catch (WorldEditException e) {
                throw new CompletionException("WorldEdit paste operation failed", e);
            }
        }, executorService).exceptionally(ex -> {
            ex.getCause();
            ClashArenaLogger.log(Level.SEVERE, String.valueOf(ex.getCause()));
            return null;
        });
    }
}